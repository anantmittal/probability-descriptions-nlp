...
class WikiCorpus(object):
    def __init__(self, wiki_dir):
        ...
        #self.dictionary.filter_tokens(bad_ids=[self.dictionary.token2id['chances','chance','probability']]) #use bad_tokens to remove words you done need.
...
corpus = WikiCorpus('../urls_data/')

mallet_path = '~/Mallet/bin/mallet'
model = LdaMallet(mallet_path, corpus, num_topics=10, id2word=corpus.dictionary, workers=4, iterations=1000, prefix="./model_files/")


model.save("url_data.model")
pickle.dump(corpus, open("url_data.corpus", "wb"))

handle = open("mallet_topics.txt", "+w")
output_str = ""

topics_as_list_of_weight_word_pairs = model.show_topics(num_topics=10, num_words=100, log=False, formatted=False)
